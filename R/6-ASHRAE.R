...
IrradianceASHRAE<-function(){
	...
	EbnASHRAE=A[1]*exp(-B[1]/sinalpha)   #Jan
	EbnASHRAE[which(Month==2)]=A[2]*exp(-B[2]/sinalpha[which(Month==2)])  #Feb
	EbnASHRAE[which(Month==3)]=A[3]*exp(-B[3]/sinalpha[which(Month==3)])  #Mar
	EbnASHRAE[which(Month==4)]=A[4]*exp(-B[4]/sinalpha[which(Month==4)])  #Apr
	EbnASHRAE[which(Month==5)]=A[5]*exp(-B[5]/sinalpha[which(Month==5)])  #May
	EbnASHRAE[which(Month==6)]=A[6]*exp(-B[6]/sinalpha[which(Month==6)])  #Jun
	EbnASHRAE[which(Month==7)]=A[7]*exp(-B[7]/sinalpha[which(Month==7)])  #Jul
	EbnASHRAE[which(Month==8)]=A[8]*exp(-B[8]/sinalpha[which(Month==8)])  #Aug
	EbnASHRAE[which(Month==9)]=A[9]*exp(-B[9]/sinalpha[which(Month==9)])  #Sep
	EbnASHRAE[which(Month==10)]=A[10]*exp(-B[10]/sinalpha[which(Month==10)])  #Oct
	EbnASHRAE[which(Month==11)]=A[11]*exp(-B[11]/sinalpha[which(Month==11)])  #Nov
	EbnASHRAE[which(Month==12)]=A[12]*exp(-B[12]/sinalpha[which(Month==12)])  #Dec
