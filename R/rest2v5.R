...
###   REST2  version5  2008
#-----##-----##-----##-----##-----##-----##-----##-----##-----##-----##-----#
###calculate GHIRest2,  according to Gueymard 2008 <REST2: High-performance solar radiation model for cloudless-sky irradiance, illuminance, and photosynthetically active radiation – Validation with a benchmark dataset >
#Input: Eext, sza,press,wv,ozone,NO2,ang_beta,ang_alpha,albedo
GHIRest2<-function(whatever){
    ...
    ama=(sinalpha+0.16851*(sza*180/pi)^0.18198/(95.318-sza*180/pi)^1.9542)^-1
    #air mass for water vapor absorption
    amw=(sinalpha+0.10648*(sza*180/pi)^0.11423/(93.781-sza*180/pi)^1.9203)^-1
    #air mass for nitrogen dioxide absorption
    amn=(sinalpha+1.1212*(sza*180/pi)^1.6132/(111.55-sza*180/pi)^3.2629)^-1
    #air mass for ozone absorption
    amo=(sinalpha+1.0651*(sza*180/pi)^0.6379/(101.8-sza*180/pi)^2.2694)^-1
    #air mass for Rayleigh scattering and uniformly mixed gases absorption
    amR=(sinalpha+0.48353*((sza*180/pi)^0.095846)/(96.741-sza*180/pi)^1.754)^-1
    amRe=(press/1013.25)*(sinalpha+0.48353*((sza*180/pi)^0.095846)/(96.741-sza*180/pi)^1.754)^-1
    ...
    Tg1=(1 + 0.95885*amRe + 0.012871*amRe^2) /(1 + 0.96321*amRe + 0.015455*amRe^2)
     ...
     Ba=1 - exp(-0.6931-1.8326*sinalpha)
     ...
     Edp1=E0n1*sinalpha*To1*Tg1*Tn1166*Tw1166*(BR1*(1-TR1)*TA1^0.25 + Ba*F1*TR1*(1-TAS1^0.25))
     #multiple reflections between the ground and the atmosphere    
     Edd1=rg*rs1*(Ebn1*sinalpha + Edp1)/(1-rg*rs1)   
	 ...
	 #the incident diffuse irradiance on a perfectly absorbing ground 
     Edp2 = E0n2*sinalpha*To2*Tg2*Tn2166*Tw2166*(BR2*(1-TR2)*TA2^0.25 + Ba*F2*TR2*(1-TAS2^0.25))
     #multiple reflections between the ground and the atmosphere
     Edd2 = rg*rs2*(Ebn2*sinalpha + Edp2)/(1-rg*rs2)

     #TOTALS BAND1+BAND2
     #direct horizontal irradiance
     Ebh=(Ebn1 + Ebn2) * sinalpha
     #diffuse horizontal irradiance
     Edh=Edp1 + Edd1 + Edp2 + Edd2
     #global horizontal irradiance
     Eghrest2 = Ebh + Edh

     #qc
     lower=0
     Eghrest2[Eghrest2<lower]=NA
     return(Eghrest2)
}
